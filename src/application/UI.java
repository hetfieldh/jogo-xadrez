...
public class UI {
	...
	public static void criarPartida(PartidaXadrez partidaXadrez, List<PecaXadrez> capturado) {
		criaTabuleiro(partidaXadrez.getPecas());
		System.out.println();
	...
	public static void criaTabuleiro(PecaXadrez[][] pecas, boolean[][] movimentosPossiveis) {
		for (int i = 0; i < pecas.length; i++) {
			System.out.print((8 - i) + " ");
			for (int j = 0; j < pecas.length; j++) {
				criaPeca(pecas[i][j], movimentosPossiveis[i][j]);
			}
			System.out.println();
		}
		System.out.println("  a b c d e f g h");
	...
	private static void imprimirPecasCapturadas(List<PecaXadrez> capturado) {
		...
		List<PecaXadrez> preta = capturado.stream().filter(x -> x.getCor() == Cor.PRETO).collect(Collectors.toList());
