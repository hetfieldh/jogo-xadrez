...
public class PartidaXadrez {
	...
	public PecaXadrez performarMovimentacaoPeca(PosicaoXadrez posicaoOrigem, PosicaoXadrez posicaoDestino) {
		...
		if (pecaMovida instanceof Peao) {
			if(pecaMovida.getCor() == Cor.BRANCO && destino.getLinha() == 0 || pecaMovida.getCor() == Cor.PRETO && destino.getLinha() == 7) {
				...
				pecaPromovida = substituirPecaPromovida("R");
			}
		}
	...
	public PecaXadrez substituirPecaPromovida(String tipoPeca) {
		...
		Posicao pos = pecaPromovida.getPosicaoXadrez().toPosicao();
		Peca p = tabuleiro.removerPeca(pos);
		pecasNoTabuleiro.remove(p);
		
		PecaXadrez novaPeca = novaPeca(tipoPeca, pecaPromovida.getCor());
	...
	private PecaXadrez novaPeca(String tipoPeca, Cor cor) {
		...
		if (tipoPeca.equals("T")) return new Torre(tabuleiro,cor);
